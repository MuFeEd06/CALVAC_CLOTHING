...
const DEFAULTS = [
  ...
  { id: 'socials',   visible: true, x: 3,  y: 60, fontSize: 9,  color: '#777777', content: 'f  in  X  play' },
...
function getVisibleCategories(settings: SiteSettings | null, categories: Category[]) {
  const pageConfigs = parsePageConfigs(settings)
  const configuredItems = Array.isArray(pageConfigs?._categoryItems) ? pageConfigs._categoryItems : []
  const hiddenSlugs = new Set(
    configuredItems
      .filter((item: any) => item?.visible === false)
      .map((item: any) => slugify(String(item?.name ?? '')))
      .filter(Boolean),
  )
  const configuredOrder = configuredItems
    .filter((item: any) => item?.visible !== false)
    .map((item: any) => slugify(String(item?.name ?? '')))
    .filter(Boolean)
  ...
  const live = categories
    .filter(category => {
      ...
      return category.name && category.slug && category.is_active !== false && !hiddenSlugs.has(category.slug) && !hiddenSlugs.has(nameSlug)
    })
    .map(category => ({ name: category.name, slug: category.slug }))

  if (configuredOrder.length === 0) return live

  const bySlug = new Map(live.flatMap(category => [[category.slug, category], [slugify(category.name), category]] as const))
  const ordered = configuredOrder.flatMap(slug => {
    const category = bySlug.get(slug)
    if (!category) return []
    bySlug.delete(slug)
    bySlug.delete(slugify(category.name))
    return [category]
  })
...
export default function Footer({ settings, categories = [] }: Props) {
  ...
  const location = settings?.contact_location || txt(cfg, 'location', 'Your Store Address\nCity, State')
  const email = settings?.contact_email || txt(cfg, 'email', 'hello@calvac.store')
  const phone = settings?.contact_phone || settings?.whatsapp_number || txt(cfg, 'phone', '+91 98765 43210')
  const hours = settings?.open_time || txt(cfg, 'hours', '08.00 - 11.00 pm')
  const copyright = txt(cfg, 'copyright', `© 2026 ${settings?.brand_name ?? 'CALVAC'}. All rights reserved.`)
  const whatsAppUrl = whatsappHref(settings?.whatsapp_number || phone)
  const instagramUrl = instagramHref(settings?.instagram_url)
  const shopCategories = getVisibleCategories(settings, categories)

  const socialLinks = [
    whatsAppUrl ? { label: 'WA', href: whatsAppUrl, title: 'WhatsApp' } : null,
    instagramUrl ? { label: 'IG', href: instagramUrl, title: 'Instagram' } : null,
    ...
    <footer style={{ background: bg, color: '#fff', fontFamily: 'Barlow, sans-serif' }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  {socialLinks.map(link => (
                    <a
                      ...
                      style={{ width: 36, height: 36, border: '1px solid rgba(255,255,255,0.2)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.72)', textDecoration: 'none' }}
                    >
                      {link.label}
                    </a>
                  ))}
          ...
          <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: 10 }}>
            {shopCategories.map(category => (
              <li key={category.slug}>
                <Link href={`/shop?category=${encodeURIComponent(category.slug)}`} style={{ fontSize: 13, color: 'rgba(255,255,255,0.55)', textDecoration: 'none' }}>{category.name}</Link>
              </li>
            ))}
            {shopCategories.length === 0 && (
              <li>
                <Link href="/shop" style={{ fontSize: 13, color: 'rgba(255,255,255,0.55)', textDecoration: 'none' }}>All Products</Link>
              </li>
            )}
          </ul>
        </div>
      ...
      <div className="footer-bottom">
        <div style={{ display: 'flex', gap: 20, flexWrap: 'wrap' }}>
          <Link href="/privacy-policy" style={{ fontSize: 11, color: 'rgba(255,255,255,0.25)', textDecoration: 'none' }}>Privacy Policy</Link>
          <Link href="/return-policy" style={{ fontSize: 11, color: 'rgba(255,255,255,0.25)', textDecoration: 'none' }}>Return Policy</Link>
          <Link href="/shipping-policy" style={{ fontSize: 11, color: 'rgba(255,255,255,0.25)', textDecoration: 'none' }}>Shipping Policy</Link>
        </div>

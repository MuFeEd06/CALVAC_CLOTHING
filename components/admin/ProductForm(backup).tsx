        ...
        <div className="space-y-5">
          ...
          <div className="bg-white rounded-2xl p-5 space-y-3">
            <div>
              ...
              <p className="text-xs text-[var(--gray-mid)] mt-1">Assign this product to a fixed slot in the homepage carousel. Leaves the slot empty if unset.</p>
            </div>
            <div>
              ...
              <div className="grid grid-cols-4 gap-2">
                ...
                <button
                  type="button"
                  onClick={() => setForm(f => ({ ...f, carousel_slot: '' }))}
                  className={`py-2 rounded-lg border text-xs font-600 transition-colors ${
                    form.carousel_slot === ''
                      ? 'bg-black text-white border-black'
                      : 'border-[var(--gray-light)] hover:border-black text-[var(--gray-mid)]'
                  }`}
                >
                  None
                </button>
                {[1,2,3,4,5,6].map(slot => (
                  <button
                    key={slot}
                    type="button"
                    onClick={() => setForm(f => ({ ...f, carousel_slot: String(slot), is_featured: true }))}
                    className={`py-2 rounded-lg border text-xs font-600 transition-colors ${
                      form.carousel_slot === String(slot)
                        ? 'bg-[var(--orange)] text-white border-[var(--orange)]'
                        : 'border-[var(--gray-light)] hover:border-[var(--orange)] hover:text-[var(--orange)]'
                    }`}
                  >
                    Card {slot}
                  </button>
              ...
              {form.carousel_slot && (
                <p className="text-xs text-[var(--orange)] mt-2 font-500">
                  ✓ Assigned to Card {form.carousel_slot} · Featured automatically enabled
                </p>
              )}
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl p-5 space-y-3">
            <div>
              ...
              <p className="text-xs text-[var(--gray-mid)] mt-1">Tag this product to a collection. Shown when visitors click a collection row on the homepage. Tag names match what's set in the admin page editor → Collections section.</p>
            </div>
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <button
                  type="button"
                  onClick={() => setForm(f => ({ ...f, collection_tag: '' }))}
                  className={`py-2 rounded-lg border text-xs font-600 transition-colors ${
                    form.collection_tag === ''
                      ? 'bg-black text-white border-black'
                      : 'border-[var(--gray-light)] hover:border-black text-[var(--gray-mid)]'
                ...
                {[
                  ...
                  'Timeless Classics 2026',
                  'Seasonal Collections 2025',
                ].map(name => {
                  const slug = name.toLowerCase().trim().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '')
                  const isActive = form.collection_tag === slug
                  return (
                    <button
                      key={slug}
                      type="button"
                      onClick={() => setForm(f => ({ ...f, collection_tag: slug }))}
                      className={`py-2 px-3 rounded-lg border text-xs font-600 transition-colors text-left ${
                        isActive
                          ? 'bg-[var(--orange)] text-white border-[var(--orange)]'
                          : 'border-[var(--gray-light)] hover:border-[var(--orange)] hover:text-[var(--orange)]'
                      }`}
              ...
              <p className="text-[10px] text-[var(--gray-mid)] mt-2 leading-relaxed">
                ⚠ If you rename a collection in the page editor, update the tag here too so products still appear correctly.
              </p>
              {form.collection_tag && (
                <p className="text-xs text-[var(--orange)] mt-1 font-500">
                  ✓ Tagged as: {form.collection_tag}
                </p>
              )}
            </div>
          </div>

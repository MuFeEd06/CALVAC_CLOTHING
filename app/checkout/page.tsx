...
function UnavailableOverlayButton({
  unavailable,
  children,
}: {
  unavailable: boolean
...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    ...
    supabase.from('site_settings').select('whatsapp_number,page_configs').single()
      .then(({ data }) => {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (authMode === 'signup') {
        const emailRedirectTo = typeof window !== 'undefined' ? `${window.location.origin}/checkout` : undefined
        ...
        if (!data.session) {
          setAuthNotice('Check your email to confirm your account, then sign in to continue checkout.')
          return
        }
      } else {
  ...
  const createValidatedOrder = async (method: PaymentMethod): Promise<Order> => {
    ...
    if (!res.ok) throw new Error(payload.error || 'Unable to create order')
  ...
  const placeOrder = async (method: PaymentMethod) => {
    ...
    try {
      const order = await createValidatedOrder(method)
      ...
      if (method === 'whatsapp') {
        ...
        clearCart()
        router.push(`/checkout/success?order=${order.order_number}&method=whatsapp`)
        return
      }

      if (method === 'cod') {
        clearCart()
        router.push(`/checkout/success?order=${order.order_number}&method=cod`)
        return
      }
        ...
        const options = {
          ...
          handler: async (response: any) => {
            try {
              ...
              clearCart()
              router.push(`/checkout/success?order=${order.order_number}&method=razorpay`)
            } catch (err: any) {
          ...
          modal: {
            ondismiss: () => {
              setError('Online payment was cancelled. Your order was not marked as paid.')
              setLoading(false)
            },
          },
        }
          ...
          <div>
                ...
                <>
                  ...
                  <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
                    {(['guest', 'login', 'signup'] as AuthMode[]).map(m => (
                      <button key={m} onClick={() => { setAuthMode(m); setAuthError(''); setAuthNotice('') }} style={{ padding: '8px 18px', borderRadius: 20, border: 'none', cursor: 'pointer', fontSize: 12, fontWeight: 600, letterSpacing: '1px', textTransform: 'uppercase', background: authMode === m ? '#0d0d0d' : '#f0f0ee', color: authMode === m ? '#fff' : '#666', transition: 'all 0.15s', fontFamily: 'Barlow, sans-serif' }}>
                    ...
                    <form onSubmit={handleAuth}>
                      {authMode === 'signup' && (
                        <div style={{ marginBottom: 14 }}>
                          ...
                          <input type="text" value={authName} onChange={e => setAuthName(e.target.value)} placeholder="Your name" required autoComplete="name" style={inp} />
                        </div>
                      )}
                      <div style={{ marginBottom: 14 }}>
                        <label style={lbl}>Email</label>
                        <input type="email" value={authEmail} onChange={e => setAuthEmail(e.target.value)} placeholder="you@example.com" required autoComplete="email" inputMode="email" style={inp} />
                      </div>
                      <div style={{ marginBottom: 16 }}>
                        <label style={lbl}>Password</label>
                        <input type="password" value={authPassword} onChange={e => setAuthPassword(e.target.value)} placeholder="••••••••" required minLength={6} autoComplete={authMode === 'signup' ? 'new-password' : 'current-password'} style={inp} />
                      </div>
                      {authError && <p style={{ color: '#c0392b', fontSize: 13, marginBottom: 12 }}>{authError}</p>}
                      {authNotice && <p style={{ color: '#166534', fontSize: 13, marginBottom: 12, lineHeight: 1.5 }}>{authNotice}</p>}
            ...
            <div style={s.card}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                <UnavailableOverlayButton unavailable={!paymentMethods.whatsapp}>
                <button
                  onClick={() => placeOrder('whatsapp')}
                  disabled={loading || !paymentMethods.whatsapp}
                  style={{ width: '100%', padding: '16px 20px', borderRadius: 12, border: '2px solid #25d366', background: loading || !paymentMethods.whatsapp ? '#f5f5f3' : '#f0fff4', cursor: loading || !paymentMethods.whatsapp ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', gap: 14, transition: 'all 0.2s', textAlign: 'left' }}
                >
                ...
                </UnavailableOverlayButton>

                {/* COD — always available */}
                <UnavailableOverlayButton unavailable={!paymentMethods.cod}>
                <button
                  onClick={() => placeOrder('cod')}
                  disabled={loading || !paymentMethods.cod}
                  style={{ width: '100%', padding: '16px 20px', borderRadius: 12, border: '2px solid #e8e8e5', background: loading || !paymentMethods.cod ? '#f5f5f3' : '#fff', cursor: loading || !paymentMethods.cod ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', gap: 14, transition: 'all 0.2s', textAlign: 'left' }}
                >
                ...
                <UnavailableOverlayButton unavailable={!paymentMethods.razorpay}>
                <button
                  onClick={() => user ? placeOrder('razorpay') : setAuthMode('login')}
                  disabled={loading || !paymentMethods.razorpay}
                  style={{ width: '100%', padding: '16px 20px', borderRadius: 12, border: `2px solid ${user ? '#0d0d0d' : '#e8e8e5'}`, background: user ? '#0d0d0d' : '#f5f5f5', cursor: loading || !paymentMethods.razorpay ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', gap: 14, transition: 'all 0.2s', textAlign: 'left', opacity: user ? 1 : 0.7 }}
                >

  ...
  return (
    <div className="p-4 md:p-8">
      <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
        ...
        <Link
          href={manageStorePath('/products/new')}
      ...
      <div className="flex gap-2 mb-6 overflow-x-auto pb-1">
        <Link href={manageStorePath('/products')} className="flex-shrink-0 px-4 py-1.5 rounded-full bg-black text-white text-xs font-600 tracking-wide">
          All ({products.length})
        </Link>
        {categories.map(cat => (
          <Link
            key={cat.id}
            href={`${manageStorePath('/products')}?category=${cat.slug}`}
            ...
            {products.length === 0 && (
              <tr>
                <td colSpan={6} className="text-center py-16 text-[var(--gray-mid)] text-sm">
                  No products yet.{' '}
                  <Link href={manageStorePath('/products/new')} className="text-[var(--orange)] underline">Add your first product</Link>
                </td>
              </tr>
            )}
              ...
              <tr key={product.id} className="border-b border-[var(--gray-light)] last:border-0 hover:bg-[#fafaf8] transition-colors">
                    ...
                    <div className="w-12 h-14 bg-[var(--gray-light)] flex-shrink-0 overflow-hidden rounded">
                      {product.images[0] ? (
                        <Image
                          ...
                          width={48}
                          height={56}
                          className={`w-full h-full ${TOP_FOCUSED_IMAGE_CLASS_NAME}`}
                        />
                      ) : (
                ...
                <td className="px-4 py-4">
                  <div className="flex items-center gap-2 justify-end">
                    <Link
                      href={manageStorePath(`/products/${product.id}/edit`)}
